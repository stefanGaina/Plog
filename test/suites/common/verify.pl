...
sub open_files() {
	...
	my $log_file_path             = $ARGV[0] . "messages";
	my $expected_log_file_path    = $ARGV[0] . "expected/messages";
	my $output_file_path          = $ARGV[0] . "output.txt";
	my $expected_output_file_path = $ARGV[0] . "expected/output.txt";

	open(log_file            , "<$log_file_path"            ) or die "Failed to open \"$log_file_path            \" in read mode!";
	open(expected_log_file   , "<$expected_log_file_path"   ) or die "Failed to open \"$expected_log_file_path   \" in read mode!";
	open(output_file         , "<$output_file_path"         ) or die "Failed to open \"$output_file_path         \" in read mode!";
	open(expected_output_file, "<$expected_output_file_path") or die "Failed to open \"$expected_output_file_path\" in read mode!";
}

sub read_files() {
	$required_index = 0;
	while (<expected_log_file>) {
